...
class PrefsDialog(Toplevel):
    def __init__(self, mainapp, prefs_array):
        ...
        cur_row = 0
        tkinter.Label(body, text='Adobe Key file (adeptkey.der)').grid(row=cur_row, sticky=tkinter.constants.E)
        ...
        cur_row = cur_row + 1
        tkinter.Label(body, text='Kindle Key file (kindlekey.k4i)').grid(row=cur_row, sticky=tkinter.constants.E)
        ...
        cur_row = cur_row + 1
        tkinter.Label(body, text='Android Kindle backup file (backup.ab)').grid(row=cur_row, sticky=tkinter.constants.E)
        ...
        cur_row = cur_row + 1
        tkinter.Label(body, text='Barnes and Noble Key file (bnepubkey.b64)').grid(row=cur_row, sticky=tkinter.constants.E)
        ...
        cur_row = cur_row + 1
        tkinter.Label(body, text='Mobipocket PID list\n(8 or 10 characters, comma separated)').grid(row=cur_row, sticky=tkinter.constants.E)
        ...
        cur_row = cur_row + 1
        tkinter.Label(body, text='eInk Kindle Serial Number list\n(16 characters, comma separated)').grid(row=cur_row, sticky=tkinter.constants.E)
        ...
        cur_row = cur_row + 1
        tkinter.Label(body, text='eReader data list\n(name:last 8 digits on credit card, comma separated)').grid(row=cur_row, sticky=tkinter.constants.E)
        ...
        cur_row = cur_row + 1
        tkinter.Label(body, text="Output Folder (if blank, use input ebook's folder)").grid(row=cur_row, sticky=tkinter.constants.E)
        ...
        cur_row = cur_row + 1
        tkinter.Label(body, text='').grid(row=cur_row, column=0, columnspan=2, sticky=tkinter.constants.N)

        cur_row = cur_row + 1
        tkinter.Label(body, text='Alternatively Process an eBook').grid(row=cur_row, column=0, columnspan=2, sticky=tkinter.constants.N)

        cur_row = cur_row + 1
        tkinter.Label(body, text='Select an eBook to Process*').grid(row=cur_row, sticky=tkinter.constants.E)
        ...
        cur_row = cur_row + 1
        tkinter.Label(body, font=("Helvetica", "10", "italic"), text='*To DeDRM multiple ebooks simultaneously, set your preferences and quit.\nThen drag and drop ebooks or folders onto the DeDRM_Drop_Target').grid(row=cur_row, column=1, sticky=tkinter.constants.E)

        cur_row = cur_row + 1
        tkinter.Label(body, text='').grid(row=cur_row, column=0, columnspan=2, sticky=tkinter.constants.E)
        ...
        self.pbotton.pack(side=tkinter.constants.LEFT)
        buttons.pack()
        self.qbotton = tkinter.Button(buttons, text="Quit", width=14, command=self.quitting)
        self.qbotton.pack(side=tkinter.constants.RIGHT)
        buttons.pack()
    ...
    def get_adkpath(self):
        ...
        adkpath = tkinter.filedialog.askopenfilename(initialdir = cpath, parent=None, title='Select Adept Key file',
            defaultextension='.der', filetypes=[('Adept Key file', '.der'), ('All Files', '.*')])
        if adkpath:
            adkpath = os.path.normpath(adkpath)
            self.adkpath.delete(0, tkinter.constants.END)
            self.adkpath.insert(0, adkpath)
        return

    def get_kkpath(self):
        cpath = self.kkpath.get()
        kkpath = tkinter.filedialog.askopenfilename(initialdir = cpath, parent=None, title='Select Kindle Key file',
            defaultextension='.k4i', filetypes=[('Kindle Key file', '.k4i'), ('All Files', '.*')])
        if kkpath:
            kkpath = os.path.normpath(kkpath)
            self.kkpath.delete(0, tkinter.constants.END)
            self.kkpath.insert(0, kkpath)
        return

    def get_akkpath(self):
        cpath = self.akkpath.get()
        akkpath = tkinter.filedialog.askopenfilename(initialdir = cpath, parent=None, title='Select Android for Kindle backup file',
            defaultextension='.ab', filetypes=[('Kindle for Android backup file', '.ab'), ('All Files', '.*')])
        if akkpath:
            akkpath = os.path.normpath(akkpath)
            self.akkpath.delete(0, tkinter.constants.END)
            self.akkpath.insert(0, akkpath)
        return

    def get_bnkpath(self):
        cpath = self.bnkpath.get()
        bnkpath = tkinter.filedialog.askopenfilename(initialdir = cpath, parent=None, title='Select Barnes and Noble Key file',
            defaultextension='.b64', filetypes=[('Barnes and Noble Key file', '.b64'), ('All Files', '.*')])
        if bnkpath:
            bnkpath = os.path.normpath(bnkpath)
            self.bnkpath.delete(0, tkinter.constants.END)
            self.bnkpath.insert(0, bnkpath)
        return

    def get_bookpath(self):
        cpath = self.bookpath.get()
        bookpath = tkinter.filedialog.askopenfilename(parent=None, title='Select eBook for DRM Removal',
            filetypes=[('All Files', '.*'),
                    ('ePub Files','.epub'),
                    ('Kindle','.azw'),
                    ('Kindle','.azw1'),
        ...
        if bookpath:
            ...
            self.bookpath.delete(0, tkinter.constants.END)
...
class ConvDialog(Toplevel):
    def __init__(self, master, prefs_array={}, filenames=[]):
        ...
        tkinter.Label(body, text='Activity Bar').grid(row=0, sticky=tkinter.constants.E)
        self.bar = ActivityBar(body, length=80, height=15, barwidth=5)
        self.bar.grid(row=0, column=1, sticky=sticky)

        msg1 = ''
        self.stext = ScrolledText(body, bd=5, relief=tkinter.constants.RIDGE, height=4, width=80, wrap=tkinter.constants.WORD)
        ...
        self.qbutton.pack(side=tkinter.constants.BOTTOM)
        self.status['text'] = ''
